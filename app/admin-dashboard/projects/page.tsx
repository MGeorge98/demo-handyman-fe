...
import { ArrowLeft, Bell, Calendar, ChevronRight, FileText, MoreHorizontal, Search, Users, Trash2, Mop, Sparkles, Clock, CalendarIcon, MapPin, Building, Plus, ChevronDown, Edit, Trash, User, Phone, Mail, Star, Briefcase, Info, Send, ChevronUp } from "lucide-react"
...
import DailyReporting from "@/app/coordinator-dashboard/daily-reporting/page"
// import ResourceManagement from "../resources/page"
// import DailyReporting from "../daily-reporting/page"
...
export default function ProjectMonitoring() {
    ...
    const ProjectDetails = useCallback(({ project, onBack }) => {
        ...
        return (
            <>
                <Button onClick={onBack} className="bg-blue-500 text-white hover:bg-blue-600 rounded-full">
                    ...
                    <CardContent className="p-0">
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                            <TabsList className="w-full justify-start rounded-none border-b bg-transparent p-0">
                                ...
                                <TabsTrigger value="team" className="rounded-t-lg data-[state=active]:bg-white data-[state=active]:shadow">Gestionare Echipe</TabsTrigger>
                                <TabsTrigger value="resources" className="rounded-t-lg data-[state=active]:bg-white data-[state=active]:shadow">Resurse</TabsTrigger>
                                ...
                                <TabsTrigger value="dailyReport" className="rounded-t-lg data-[state=active]:bg-white data-[state=active]:shadow">Raportare Zilnică</TabsTrigger>
    ...
    function TeamManagement({ project }) {
        const initialTeams = [
            { id: 1, name: "Echipa Alpha", members: [1, 2] },
            { id: 2, name: "Echipa Beta", members: [3, 4] },
        ]
        ...
        const handleAddTeam = () => {
            if (newTeamName.trim() === "") {
                toast({
                    title: "Eroare",
                    description: "Numele echipei nu poate fi gol.",
                    variant: "destructive",
                })
                return
            }
            ...
            toast({
                title: "Echipă adăugată",
                description: `Echipa "${newTeamName}" a fost creată cu succes.`,
            })
        }

        const handleRemoveFromTeam = (teamId, memberId) => {
            ...
            toast({
                ...
                description: "Membrul a fost eliminat din echipă cu succes.",
            })
        }
        ...
        const handleAddMember = () => {
            if (!selectedEmployeeId) {
                toast({
                    title: "Eroare",
                    description: "Vă rugăm să selectați un angajat.",
                    variant: "destructive",
                })
                return
            }
            setTeams(teams.map(team =>
                team.id === selectedTeamForMember.id
                    ? { ...team, members: [...team.members, parseInt(selectedEmployeeId)] }
                    : team
            ))
            ...
            toast({
                ...
                description: "Noul membru a fost adăugat în echipă cu succes.",
            })
        }

        const openEmployeeModal = (employee) => {
            // Implement employee details modal logic here
            console.log("Open employee details for:", employee)
        }

        return (
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-800">Gestionare Echipe</h2>
                    <Button onClick={() => setIsAddTeamModalOpen(true)} className="bg-blue-500 text-white hover:bg-blue-600 rounded-full">
                        <Plus className="mr-2 h-4 w-4" /> Adaugă Echipă
                    </Button>
                </div>

                {teams.map((team) => (
                    <Card key={team.id} className="overflow-hidden rounded-2xl border-none shadow-lg">
                        <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-800 text-white">
                            <div className="flex justify-between items-center">
                                ...
                                >
                                    {expandedTeam === team.id ? <ChevronUp className="h-6 w-6" /> : <ChevronDown className="h-6 w-6" />}
                                </Button>
                            </div>
                                        ...
                                        {team.members.map((memberId) => {
                                            ...
                                            return (
                                                <TableRow key={member.id}>
                                                    ...
                                                    <TableCell>
                                                        <div className="flex space-x-2">
                                                            <Button variant="destructive" onClick={() => handleRemoveFromTeam(team.id, member.id)} className="rounded-full">
                                                                Elimină
                                                            </Button>
                                                            <Button
                ...
                <Dialog open={isAddTeamModalOpen} onOpenChange={setIsAddTeamModalOpen}>
                    <DialogContent className="sm:max-w-[425px] rounded-2xl">
                        <DialogHeader>
                            <DialogTitle>Adaugă Echipă Nouă</DialogTitle>
                            <DialogDescription>
                                Introduceți numele noii echipe pentru proiect.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <DialogFooter>
                            <Button onClick={handleAddTeam} className="bg-blue-500 text-white hover:bg-blue-600 rounded-full">
                                Adaugă Echipa
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                ...
                <Dialog open={isAddMemberModalOpen} onOpenChange={setIsAddMemberModalOpen}>
                    <DialogContent className="sm:max-w-[425px] rounded-2xl">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Selectați un angajat pentru a-l adăuga în echipă.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            <div className="grid grid-cols-4 items-center gap-4">
                                ...
                                <Select value={selectedEmployeeId} onValueChange={setSelectedEmployeeId}>
                                    <SelectTrigger className="col-span-3 rounded-full">
                                        <SelectValue placeholder="Selectați un angajat" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {employees.filter(emp => !selectedTeamForMember?.members.includes(emp.id)).map((employee) => (
                        ...
                        <DialogFooter>
                            <Button onClick={handleAddMember} className="bg-blue-500 text-white hover:bg-blue-600 rounded-full">
                                Adaugă în Echipă
                            </Button>
                        </DialogFooter>
                    </DialogContent>
    ...
    return (
        <DashboardLayout links={adminLinks} title="Proiecte">
